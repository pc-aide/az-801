function attachSolutionButtonListeners_question2(button) {
    button.addEventListener('click', function() {
        ...
        if (selectedAnswer && selectedAnswer.value === 'B. No') {
            document.querySelector('input[name="question2"][value="B. No"]').parentElement.classList.add('highlight');
        } else {
            document.querySelector('input[name="question2"][value="B. No"]').parentElement.classList.add('incorrect');
            allCorrect = false;
        }
